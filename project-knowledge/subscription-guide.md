...
| **Цена** | $0 | $0 (первые 7 дней) | $9.99/мес |
| **Срок действия** | бессрочно | 7 дней | ежемесячно |
| **Анализ питания** | ✓ | ✓ | ✓ |
| **Умный поиск продуктов** | ✓ | ✓ | ✓ |
| **AI агент (ADVANCED_AI_TOOLS)** | ✓ | ✓ | ✓ |
| **Генерация рецептов** | ✓\* | ✓ | ✓ |

\* `RECIPE_GENERATION` формально определён только в Pro-плане, но в коде нигде не проверяется — Free-пользователи пользуются им через общую квоту токенов.
...
Токены триала — **одноразовый пул**, не возобновляемый ежемесячно. Если пользователь за 7 дней потратит все 300 000 токенов — AI заблокируется раньше, чем истечёт время. Если не потратит — всё равно после 7 дней план переключится на Free.
...
- AI агент (ADVANCED_AI_TOOLS) **остаётся доступен** — входит в Free plan
- Анализ питания и умный поиск продуктов остаются доступны
- Генерация рецептов технически тоже работает (код не проверяет RECIPE_GENERATION)
